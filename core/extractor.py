...
def extract_action_items(transcript:str)->str:
    chain = build_chain("You are an expert meeting analyst. From the meeting transcript, "
            "extract aall action itmes. For each provide:\n"
            "- Task description\n"
            "-Owner (who is responsible)\n"
            "- Deadline (if mentioned, else write 'Not specified')\n\n"
            "Format as a mentioned list. If none found say 'No action items found."
                        )
...
def extract_questions(transcript:str)->str:
    chain = build_chain(
        ...
        "or the topics needing follow-up. format as numbered list."
        "if none found say 'No unresolved questions found."
    )
    return chain.invoke(transcript)